...
#define MIDIEVENT(at,t,ch,pa,pb) \
  new_=(MidiEventList *) safe_malloc(sizeof(MidiEventList)); \
  new_->event.time=at; new_->event.type=t; new_->event.channel=ch; \
  new_->event.a=pa; new_->event.b=pb; new_->next=0;\
  return new_;
...
static int read_track(int append)
{
  ...
  for (;;)
    {
      if (!(new_=read_midi_event())) /* Some kind of error  */
	return -2;

      if (new_==MAGIC_EOT) /* End-of-track Hack. */
	{
          pos = SDL_RWtell(rw);
          if (pos < next_pos)
      ...
      while (next && (next->event.time < new_->event.time))
	{
	  meep=next;
	  next=(MidiEventList *) meep->next;
	}
