...
struct _MEMTRN {                        /*  Transaction info                 */
    ...
    word     line;                      /*  Line allocation was from         */
...
static void mem_tag_err      (void *ptr, const char *filename, word lineno);
static void mem_scavenge     (void);
static void mem_del_trans    (MEMTRN *trn);
static void mem_display_list (MEMHDR *ptr, FILE *fp);
static void mem_check_list   (MEMHDR *ptr, const char *filename, word lineno);
...
static void
mem_tag_err (void *ptr, const char *filename, word lineno)
{
    fprintf (stdout, "Memory tag error - %p - %s (%d)\n",
                     ptr, filename? filename: "<Unknown>", lineno);
    fflush  (stdout);
    abort ();
}
...
)
{
    ...
    if (string)                         /*  If string not null, copy it      */
      {
        copy = mem_alloc_ (trn, strlen (string) + 1, filename, lineno);
        if (copy)
            strcpy (copy, string);
      }
    else
...
#   if (defined (MEM_TRACE))
    trace ("%s (%d): allocate descr=%p", 
           filename? filename: "-", lineno, descr);
#   endif
...
{
#   if (defined (MEM_TRACE))
    trace ("%s (%d): rollback transaction", 
           filename? filename: "-", lineno);
#   endif
    ...
    while (ptr != lst)
      {
        ...
        fprintf (fp, "  %s (%d)",
                 ptr-> file? ptr-> file: "<Unknown>", ptr-> line);
        if (ptr-> tag != MEMTAG)
            fprintf (fp, " INVALID");
