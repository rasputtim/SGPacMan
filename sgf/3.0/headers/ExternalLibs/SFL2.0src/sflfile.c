...
static Bool   system_devicename   (char *filename);
#endif
...
{
#if (defined (MSDOS_FILESYSTEM))
    if (system_devicename ((char *) filename))
 ...
 */
static Bool
system_devicename (char *filename)
{
    char
        *char_ptr,
        *token,
    ...
    filename = mem_strdup (filename);   /*  Copy it so we can play with it   */
...
/*  ---------------------------------------------------------------------[<]-
    ...
    Synopsis: Copies the contents of dest onto src.  If src does not exist,
    ...
    if (search_curdir && file_exists (work_name))
      {
        if (fully_specified (work_name))
          ...
          {
            curdir = get_curdir ();
            sprintf (full_name, "%s%s", curdir, work_name);
            mem_free (curdir);
          }
#if (defined (MSDOS_FILESYSTEM))
        strconvch (full_name, '/', '\\');
...
#else
    return (file_exists (arg_filename));
#endif
}
