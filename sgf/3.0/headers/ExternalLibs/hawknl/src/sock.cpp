 ...
 */
static SGF::Util::Thread::Lock portlock;
...
static NLushort sock_getNextPort(void)
{
    ...
    SGF::Util::Thread::acquireLock(&portlock);
    ...
    SGF::Util::Thread::releaseLock(&portlock);
    return nextport;
}
...
static NLint sock_bind(SOCKET socket, const struct sockaddr *a, int len)
{
    ...
    while(ntries-- > 0)
    {
		...
		//NLint res=;
        if(bind(socket, (const struct sockaddr *)addr, len) != SOCKET_ERROR)
        {
            found = NL_TRUE;
            break;
        }
...
static SGF::Util::Thread::Lock rpMutex;                /* mutex to lock the functions */
...
{
    SGF::Util::Thread::acquireLock(&rpMutex);
    (void)nlGroupAddSocket(rpGroup, socket);
    rpSocketCount++;
    if(needThread == NL_TRUE){
        SGF::Util::Thread::ThreadId_t thread;
         ...
         */
        SGF::Util::Thread::createThread(&thread, NULL, (SGF::Util::Thread::ThreadFunction) sock_rpThread, NULL);
        needThread = NL_FALSE;
    }
    SGF::Util::Thread::releaseLock(&rpMutex);
}
...
NLboolean sock_Init(void)
{
    ...
    if(SGF::Util::Thread::initializeLock(&portlock) != true)
    {
        nlSetError(NL_SYSTEM_ERROR);
        return NL_FALSE;
    }
#endif
    if(SGF::Util::Thread::initializeLock(&rpMutex) != true)
    {
        nlSetError(NL_SYSTEM_ERROR);
        return NL_FALSE;
    }
...
#ifdef HL_WINDOWS_APP
    (void)WSACleanup();
    SGF::Util::Thread::destroyLock(&portlock);
    SGF::Util::Thread::destroyLock(&rpMutex);
...
static NLboolean sock_ConnectUDP(NLsocket socket, const NLaddress *address)
{
    ...
    while((time(&t) - begin) < 6)
    {
        ...
        SGF::Util::rest(NL_CONNECT_SLEEP);
    }
...
static NLboolean sock_ConnectUDPAsynch(NLsocket socket, const NLaddress *address)
{
    ...
    sock->conerror = NL_FALSE;
    SGF::Util::Thread::ThreadId_t thread;
    if (!SGF::Util::Thread::createThread(&thread, NULL, (SGF::Util::Thread::ThreadFunction) sock_ConnectUDPAsynchInt, (void *)addr)){
        return NL_FALSE;
    }
    return NL_TRUE;
}
...
NLboolean sock_GetNameFromAddrAsync(const NLaddress *address, NLchar *name)
{
    ...
    addr->name = name;
    SGF::Util::Thread::ThreadId_t thread;
    if (!SGF::Util::Thread::createThread(&thread, NULL, (SGF::Util::Thread::ThreadFunction) sock_GetNameFromAddrAsyncInt, (void *)addr)){
        return NL_FALSE;
    }
    return NL_TRUE;
}
...
NLboolean sock_GetAddrFromNameAsync(const NLchar *name, NLaddress *address)
{
    ...
    SGF::Util::Thread::ThreadId_t thread;
    if (!SGF::Util::Thread::createThread(&thread, NULL, (SGF::Util::Thread::ThreadFunction) sock_GetAddrFromNameAsyncInt, (void *)addr)){
        return NL_FALSE;
    }
    return NL_TRUE;
}
