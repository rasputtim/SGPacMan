...
static SGF::Util::Thread::Lock grouplock;
...
void nlGroupLock(void){
    SGF::Util::Thread::acquireLock(&grouplock);
}

void nlGroupUnlock(void){
    SGF::Util::Thread::releaseLock(&grouplock);
}
...
NLboolean nlGroupInit(void)
{
    ...
    if (!SGF::Util::Thread::initializeLock(&grouplock)){
...
void nlGroupShutdown(void)
{
    ...
    SGF::Util::Thread::destroyLock(&grouplock);
}
...
HL_EXP NLint HL_APIENTRY nlGroupCreate(void)
{
    ...
    if(SGF::Util::Thread::acquireLock(&grouplock) != 0)
    {
        nlSetError(NL_SYSTEM_ERROR);
        return NL_INVALID;
    }
    ...
    if(SGF::Util::Thread::releaseLock(&grouplock) != 0)
    {
        nlSetError(NL_SYSTEM_ERROR);
        return NL_INVALID;
    }
