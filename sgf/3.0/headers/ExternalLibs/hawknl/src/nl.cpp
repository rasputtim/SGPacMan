...
#include <string.h>
#include "../hawknl/hawknl/nlinternal.h"
#include "../hawknl/hawknl/sock.h"
#include "../hawknl/hawknl/serial.h"
#include "../hawknl/hawknl/parallel.h"
...
#include "../../ExternalLibs/hawknl/hawknl/loopback.h"
#endif

#if defined HL_WINDOWS_APP && defined NL_INCLUDE_IPX
  #include "../../ExternalLibs/hawknl/hawknl/ipx.h"
#endif
...
static SGF::Util::Thread::Lock socklock, instatlock, outstatlock;
...
NLsocket nlGetNewSocket(void)
{
    ...
    if(SGF::Util::Thread::acquireLock(&socklock) != 0)
    {
    ...
    if(nlNumsockets == nlMaxNumsockets)
    {
        ...
        if(temp == NULL)
        {
            SGF::Util::Thread::releaseLock(&socklock);
    ...
    if(nlNumsockets == (NLint)nlNextsocket)
    {
        ...
        if(SGF::Util::Thread::initializeLock(&sock->readlock) != true || SGF::Util::Thread::initializeLock(&sock->writelock) != true)
        {
            nlSetError(NL_SYSTEM_ERROR);
            SGF::Util::Thread::releaseLock(&socklock);
            return NL_INVALID;
    ...
    {
        NLsocket    i;
        SGF::Util::Thread::Lock readlock, writelock;
...
void nlFreeSocket(NLsocket socket)
{
    ...
    if(sock != NULL)
    {
        ...
        SGF::Util::Thread::destroyLock(&sock->readlock);
        SGF::Util::Thread::destroyLock(&sock->writelock);
        free(sock);
    }
}
...
NLboolean nlLockSocket(NLsocket socket, NLint which)
{
    ...
    if((which&NL_READ) > 0)
    {
        if(SGF::Util::Thread::acquireLock(&sock->readlock) != 0)
        {
            nlSetError(NL_SYSTEM_ERROR);
            return NL_FALSE;
        }
    ...
    if((which&NL_WRITE) > 0)
    {
        if(SGF::Util::Thread::acquireLock(&sock->writelock) != 0)
        {
            if((which&NL_READ) > 0)
            {
                SGF::Util::Thread::releaseLock(&sock->readlock);
            }
            nlSetError(NL_SYSTEM_ERROR);
            return NL_FALSE;
        }
...
void nlUnlockSocket(NLsocket socket, NLint which)
{
    ...
    if((which&NL_WRITE) > 0){
        SGF::Util::Thread::releaseLock(&sock->writelock);
    }
    if((which&NL_READ) > 0)
    {
        SGF::Util::Thread::releaseLock(&sock->readlock);
...
static void nlUpdateInStats(NLint nbytes, NLint npackets)
{
    ...
    SGF::Util::Thread::acquireLock(&instatlock);
    nlUpdateStats(&nlInstats, nbytes, npackets);
    SGF::Util::Thread::releaseLock(&instatlock);
}
...
static void nlUpdateOutStats(NLint nbytes, NLint npackets)
{
    ...
    SGF::Util::Thread::acquireLock(&outstatlock);
    nlUpdateStats(&nlOutstats, nbytes, npackets);
    SGF::Util::Thread::releaseLock(&outstatlock);
}
...
HL_EXP NLboolean HL_APIENTRY nlInit(void)
{
    ...
    if(nlInitCount == 0)
    {
        ...
        if(SGF::Util::Thread::initializeLock(&socklock) != true || SGF::Util::Thread::initializeLock(&instatlock) != true ||
            SGF::Util::Thread::initializeLock(&outstatlock) != true)
        {
            nlSetError(NL_SYSTEM_ERROR);
            nlShutdown();
            return NL_FALSE;
...
HL_EXP void HL_APIENTRY nlShutdown(void)
{
    ...
    if(driver != NULL)
    {
        ...
        SGF::Util::Thread::acquireLock(&socklock);
        if(nlSockets != NULL)
        {
            NLsocket s;
    ...
    nlGroupShutdown();
    /* destroy the mutexes */
    SGF::Util::Thread::destroyLock(&socklock);
    SGF::Util::Thread::destroyLock(&instatlock);
    SGF::Util::Thread::destroyLock(&outstatlock);
}
...
HL_EXP NLboolean HL_APIENTRY nlClose(NLsocket socket)
{
    if(driver)
    {
	    ...
	    */
            ...
            // nlUnlockSocket(socket, NL_BOTH);
            if(SGF::Util::Thread::releaseLock(&socklock) != 0)
            {
                nlSetError(NL_SYSTEM_ERROR);
                return NL_FALSE;
            }
...
HL_EXP NLboolean HL_APIENTRY nlClear(NLenum name)
{
    ...
    case NL_PACKETS_SENT:
        if(SGF::Util::Thread::acquireLock(&outstatlock) != 0){
            SGF::Util::Thread::releaseLock(&outstatlock);
            nlSetError(NL_SYSTEM_ERROR);
            return NL_FALSE;
        }
    ...
    case NL_PACKETS_RECEIVED:
        if(SGF::Util::Thread::acquireLock(&instatlock) != 0){
            SGF::Util::Thread::releaseLock(&instatlock);
            nlSetError(NL_SYSTEM_ERROR);
            return NL_FALSE;
        }
