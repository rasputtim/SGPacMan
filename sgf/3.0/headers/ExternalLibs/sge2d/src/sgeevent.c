...
static int sgeKeyCheck(SDL_Event *e, enum sgeKey key, enum sgeJoy joy, int type) {
	if (
		...
		if (
				((type==PRESSED) && (e->type==SDL_KEYDOWN) && (e->key.keysym.sym==(SDLKey)key)) ||
				((type==PRESSED) && (e->type==SDL_JOYBUTTONDOWN) && (e->jbutton.button==joy))
		) {
			return 1;
		}
		if (
				((type==RELEASED) && (e->type==SDL_KEYUP) && (e->key.keysym.sym==(SDLKey)key)) ||
...
{
	if ((event->type==SDL_KEYDOWN && event->key.keysym.sym==(SDLKey)key) || (event->type==SDL_JOYBUTTONDOWN && event->jbutton.button==joy)) {
		input->held = 1;
		input->pressed =1;
	}
	if ((event->type==SDL_KEYUP && event->key.keysym.sym==(SDLKey)key) || (event->type==SDL_JOYBUTTONUP && event->jbutton.button==joy)) {
		input->held = 0;
		input->released = 1;
	}
}
