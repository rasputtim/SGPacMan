...
SGESPRITE *sgeSpriteNewFileRange(SGEFILE *f, const char *templ, Uint32 start, Uint32 end) {
	SGESPRITE *ret=sgeSpriteNew();
	sgeSpriteAddFileRange(ret, f, templ, start, end);
	return ret;
}
...
void sgeSpriteAddSDLSurface(SGESPRITE *s, SDL_Surface *surface) {
	SGESPRITEIMAGE *new_;
	sgeNew(new_, SGESPRITEIMAGE);
	sgeSpriteImageSetImage(new_,surface);
	sgeSpriteAddSpriteImage(s,new_);
}
...
void sgeSpriteAddFile(SGESPRITE *s, SGEFILE *f, const char *name) {
	...
	sgeNew(new_, SGESPRITEIMAGE);
	sgeSpriteImageSetImage(new_, img);
	sgeArrayAdd(sgeSpriteGetCurrentSpriteArray(s), (void *)new_);
	sgeSpriteSetNumberOfFrames(s, sgeSpriteGetNumberOfFrames(s)+1);
}
...
void sgeSpriteAddFileRange(SGESPRITE *s, SGEFILE *f, const char *templ, Uint32 start, Uint32 end) {
	...
	for (i=start;i<=end;i++) {
		...
		snprintf(buf, MAXFILENAMELEN, templ, i);
