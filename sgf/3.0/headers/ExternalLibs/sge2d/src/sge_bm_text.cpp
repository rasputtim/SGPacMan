...
sge_bmpFont* sge_BF_CreateFont(SDL_Surface *surface, Uint8 flags)
{
	...
	if(!(flags&SGE_BFNOCONVERT) && !(flags&SGE_BFSFONT)){    /* Get a converted copy */
		font->FontSurface = SDL_ConvertSurface(surface,SGF::Colors::getCurrentPixelFormat(),SDL_SWSURFACE);
		...
		font->FontSurface = SDL_ConvertSurface(surface, surface->format,SDL_SWSURFACE);
	...
	if(!(flags&SGE_BFSFONT)){  /* Fixed width font */
		...
		Uint32 color = SGF::Colors::getPixel(fnt,0,0);

		while (x<fnt->w && font->Chars<256){
			if (SGF::Colors::getPixel(fnt,x,0)==color) {
				font->CharPos[i++]=x;
				
				while (x<fnt->w-1 && SGF::Colors::getPixel(fnt,x,0)==color)
					x++;
	...
	font->bcolor=SGF::Colors::getPixel(font->FontSurface,0,font->FontSurface->h-1);
	if (SDL_MUSTLOCK(font->FontSurface) && _sge_lock) {
	...
	if(flags&SGE_BFTRANSP || flags&SGE_BFSFONT)
		...
		#else
		SDL_SetColorKey(font->FontSurface,SDL_TRUE|SDL_RLEACCEL, font->bcolor);
		#endif
...
sge_bmpFont* sge_BF_OpenFont(const char *file, Uint8 flags)
{
	...
	{
		SGF::Debug::debug(SGF::Debug::error,__FUNCTION__) <<"SGE - Couldn't load font file: " <<file << endl;
		return NULL;
	}
...
SDL_Rect sge_BF_textout(SDL_Surface *surface, sge_bmpFont *font, const char *string, Sint16 x, Sint16 y, int length, bool doupdate)
{
	...
	for(int i=0; i<characters; i++)
	{
		if(!font->CharPos) /* Fixed width */
			...
			//asc code ' ' =32 // primeiro char asc = 33
...
SDL_Rect sge_BF_textoutf(SDL_Surface *surface, sge_bmpFont *font, Sint16 x, Sint16 y ,const char *format, ...)
{
	...
	#ifdef __WIN32__
	va_start(ap, format); //va_start((va_list*)ap, format); //Stupid w32 crosscompiler
	#else
	va_start(ap, format);
	#endif
...
void sge_BF_SetColor(sge_bmpFont *font, Uint8 R, Uint8 G, Uint8 B)
{
	...
	if(!font->FontSurface->format->palette){  //Slow truecolor version
		...
		//We do not support 8bbp surfaces yet
	}
}
