...
#ifndef __SGF_LIST_H__
#define __SGF_LIST_H__
...
SGF_INLINE_FORCED int CListSortCompare( const ItemType *a, const ItemType *b ) {
	assert( 0 );
	return 0;
}
#else
...
template< class ItemType >
SGF_INLINE_FORCED ItemType *CListNewElement() {
	return new ItemType;
}
...
*/
template< class ItemType >
SGF_INLINE_FORCED void swapElements( ItemType &a, ItemType &b ) {
	ItemType c = a;
	a = b;
	b = c;
}
...
public:
	...
	int				getNum() const;									// returns number of elements in list
	
	//! retorna o número de elementos da lista
	int				getNumElements() const;	// returns number of elements in list
	//! retorna o número de elementos alocadaos	
	int				getNumAllocated() const;							// returns number of elements allocated for

	void			setGranularity( int newgranularity );				// set new granularity
	int				getGranularity() const;						// get the current granularity

	size_t			getAllocatedMem() const;							// returns total size of allocated memory
	size_t			getSize() const;									// returns total size of allocated memory including size of list ItemType
	size_t			getMemoryUsed() const;							// returns size of the used elements in the list
	...
	void			assureSize( int newSize, const ItemType &initValue );	// assure list has given number of elements and initialize any new elements
	...
	ItemType *			getPtr();										// returns a pointer to the list
	const ItemType *	getPtr() const;									// returns a pointer to the list
	ItemType &			alloc();										// returns reference to a new data element at the end of the list
	int				append( const ItemType & obj );							// append element
	int				append( const CList<ItemType> &other );				// append list
	int				addUnique( const ItemType & obj );						// add unique element
	int				insert( const ItemType & obj, int index = 0 );			// insert the element at the given index
	...
	int				getIndexOf( const ItemType *obj ) const;					// returns the index for the pointer to an element in the list
	...
	void			swapList( CList<ItemType> &other );						// swap the contents of the lists
...
*/
template< class ItemType >
SGF_INLINE_FORCED CList<ItemType>::CList( int newgranularity ) {
...
*/
template< class ItemType >
SGF_INLINE_FORCED CList<ItemType>::CList( const CList<ItemType> &other ) {
	m_pData = NULL;
	*this = other;
}
...
*/
template< class ItemType >
SGF_INLINE_FORCED CList<ItemType>::~CList() {
	clear();
}
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::clear() {
	if ( m_pData ) {
		delete[] m_pData;
	}
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::deleteContents( bool p_clear ) {
	int i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED size_t CList<ItemType>::getAllocatedMem() const {
...
*/
template< class ItemType >
SGF_INLINE_FORCED size_t CList<ItemType>::getSize() const {
...
*/
template< class ItemType >
SGF_INLINE_FORCED size_t CList<ItemType>::getMemoryUsed() const {
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::getNum() const {
	return m_iNum;
}
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::getNumElements() const {
	return getNum();
}
/*
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::getNumAllocated() const {
	return m_iSize;
}
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::setNum( int newnum, bool resizes ) {
	assert( newnum >= 0 );
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::setGranularity( int newgranularity ) {
	int newsize;
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::getGranularity() const {
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::condense() {
	if ( m_pData ) {
		if ( m_iNum ) {
			resize( m_iNum );
		} else {
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::resize( int newsize ) {
	ItemType	*temp;
	int		i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::resize( int newsize, int newgranularity ) {
	ItemType	*temp;
	int		i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::assureSize( int newSize ) {
	int newNum = newSize;
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::assureSize( int newSize, const ItemType &initValue ) {
	int newNum = newSize;
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::assureSizeAlloc( int newSize, new_t *allocator ) {
	int newNum = newSize;
...
*/
template< class ItemType >
SGF_INLINE_FORCED CList<ItemType> &CList<ItemType>::operator=( const CList<ItemType> &other ) {
	int	i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED const ItemType &CList<ItemType>::operator[]( int index ) const {
	assert( index >= 0 );
	assert( index < m_iNum );
...
*/
template< class ItemType >
SGF_INLINE_FORCED ItemType &CList<ItemType>::operator[]( int index ) {
	assert( index >= 0 );
	assert( index < m_iNum );
...
*/
template< class ItemType >
SGF_INLINE_FORCED ItemType *CList<ItemType>::getPtr() {
	return m_pData;
}
...
*/
template< class ItemType >
const SGF_INLINE_FORCED ItemType *CList<ItemType>::getPtr() const {
	return m_pData;
}
...
*/
template< class ItemType >
SGF_INLINE_FORCED ItemType &CList<ItemType>::alloc() {
	if ( !m_pData ) {
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::append( ItemType const & obj ) {
	if ( !m_pData ) {
		resize( m_iGranularity );
	}
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::insert( ItemType const & obj, int index ) {
	if ( !m_pData ) {
		resize( m_iGranularity );
	}
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::append( const CList<ItemType> &other ) {
	if ( !m_pData ) {
		if ( m_iGranularity == 0 ) {	// this is a hack to fix our memset classes
			m_iGranularity = 16;
		}
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::addUnique( ItemType const & obj ) {
	int index;
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::findIndex( ItemType const & obj ) const {
	int i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED ItemType *CList<ItemType>::find( ItemType const & obj ) const {
	int i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::findNull() const {
	int i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED int CList<ItemType>::getIndexOf( ItemType const *objptr ) const {
	int index;
...
*/
template< class ItemType >
SGF_INLINE_FORCED bool CList<ItemType>::removeIndex( int index ) {
	int i;
...
*/
template< class ItemType >
SGF_INLINE_FORCED bool CList<ItemType>::remove( ItemType const & obj ) {
	int index;
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::sort( cmp_t *compare ) {
	if ( !m_pData ) {
		return;
	}
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::sortSubSection( int startIndex, int endIndex, cmp_t *compare ) {
	if ( !m_pData ) {
		return;
	}
	if ( startIndex < 0 ) {
...
*/
template< class ItemType >
SGF_INLINE_FORCED void CList<ItemType>::swapList( CList<ItemType> &other ) {
