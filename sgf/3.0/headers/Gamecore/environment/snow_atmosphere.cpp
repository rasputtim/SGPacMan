...
static void drawFlake0( Flake * f, CBitmap * work ){
    int c = (int)(200 + 3 * log((double)(f->y < 1 ? 1 : 2 * f->y)));
    if (c > 255){
        c = 255;
    }
    int color = CBitmap::makeColor(c, c, c);
...
void SnowAtmosphere::drawBackground(CBitmap * work, int x){
}

void SnowAtmosphere::drawForeground(CBitmap * work, int x){
}
        
void SnowAtmosphere::drawFront(CBitmap * work, int x){
}

void SnowAtmosphere::drawScreen(CBitmap * work, int x){
	for ( vector< Flake * >::iterator it = flakes.begin(); it != flakes.end(); it++ ){
		Flake * f = *it;
		switch ( f->type ){
			default : {
