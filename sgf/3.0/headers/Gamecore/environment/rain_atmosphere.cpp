...
playing( false ){
	...
	int colors[ 2 ];
	colors[ 0 ] = CBitmap::makeColor( 0x22, 0x66, 0x66 );
	colors[ 1 ] = CBitmap::makeColor( 0x11, 0x44, 0x77 );
	for ( int i = 0; i < 100; i++ ){
...
void RainAtmosphere::drawBackground(CBitmap * work, int x){
    const int bluish = CBitmap::makeColor(106, 184, 225);
    CBitmap::transBlender(0, 0, 0, 64);
...
void RainAtmosphere::drawForeground(CBitmap * work, int x){
}
	
void RainAtmosphere::drawFront(CBitmap * work, int x){
}
    
void RainAtmosphere::interpret(Token * message){
}

void RainAtmosphere::drawScreen(CBitmap * work, int x){
    for ( vector< Drop * >::iterator it = rain_drops.begin(); it != rain_drops.end(); it++ ){
        Drop * d = *it;
